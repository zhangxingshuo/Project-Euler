# In the 10x20 grid below, four numbers along a diagonal line have been marked in red.
...
M = [[int(j) for j in k] for k in [i.split() for i in L]]

diags_down = [np.diagonal(M,i).tolist() for i in xrange(-16,17)]
rows = [row for row in M]
cols = [[row[i] for row in M] for i in xrange(len(M))]

M = np.rot90(M)
diags_up = [np.diagonal(M,i).tolist() for i in xrange(-16,17)]

max=0
for row in rows:
	for i in xrange(16):
		row_prod = row[i]*row[i+1]*row[i+2]*row[i+3]
		if row_prod > max:
			max = row_prod
for col in cols:
	for i in xrange(16):
		col_prod = col[i]*col[i+1]*col[i+2]*col[i+3]
		if col_prod > max:
			max = col_prod
for diag in diags_down:
	i = 0
	while i <= len(diag) - 4:
		prod = diag[i]*diag[i+1]*diag[i+2]*diag[i+3]
		if prod > max:
			max = prod
		i += 1
for diag in diags_up:
	i = 0
	while i <= len(diag) - 4:
		prod = diag[i]*diag[i+1]*diag[i+2]*diag[i+3]
		if prod > max:
